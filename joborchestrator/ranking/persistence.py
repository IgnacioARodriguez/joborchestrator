...
def rank_and_save_jobs(jobs: pd.DataFrame, profile_path: str | None = None) -> dict[str, int]:
    ...
    for row in jobs.to_dict("records"):
        ranking = rank_job(row, profile)
        db.save_job_ranking(int(row["id"]), ranking)
        summary[ranking.decision] += 1
    return summary


def rank_unranked_jobs(limit: int = 500) -> dict[str, int]:
    jobs = db.get_unranked_jobs(ranking_version=RANKING_VERSION, limit=limit)
    return rank_and_save_jobs(jobs)


def rerank_all_jobs(limit: int = 1000) -> dict[str, int]:
    jobs = db.get_job_postings(limit=limit)
    return rank_and_save_jobs(jobs)


def rerank_selected_jobs(job_ids: Iterable[int]) -> dict[str, int]:
    rows = []
    for job_id in job_ids:
        job = db.get_job_posting(int(job_id))
        if job:
            rows.append(job)
    return rank_and_save_jobs(pd.DataFrame(rows))
