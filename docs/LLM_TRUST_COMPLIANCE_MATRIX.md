...
- Materials follow-up: application kit validation now rejects recruiter messages over the same 320-character limit used by golden evals. A live NVIDIA materials v3 probe on 2026-07-27 regenerated 4 raw real-job cases in memory and passed 4/4 application-materials evals plus 4/4 ATS CV evals automatically, but external qualitative review found a false positive: the PSS/serverless case avoided the exact phrase `Serverless Architecture` while still claiming AWS Lambda/DynamoDB/API Gateway. Materials v4 made validation reject avoid-overclaiming entries as claim families and expands serverless aliases/components across ATS CV and non-CV materials; a PSS-only v4 rerun failed closed on AWS Lambda/DynamoDB. Materials v5 exposed those alias lists to the generation contracts as explicit constraints and expanded slash-separated families such as `Terraform/AWS CDK/CloudFormation`. Materials v7 adds per-employer supported technology constraints and role-specific technology attribution validation. A 4-case v7 live rerun passed 4/4 materials, 4/4 ATS CV, 4/4 drift-free, and no forbidden aliases present. Persisted golden baseline still evaluates 0 materials/ATS cases because the reviewed seed fixtures are synthetic and not DB-backed.
...
| --- | --- | --- | --- | --- |
| Prompt registry | Active prompt versions are explicit and shared | Green | Registry points ranking to v6, materials to v7, and judge to v1; v7 exposes avoid-overclaiming alias families and per-employer technology constraints | Need a full v6 production rerank and DB-backed reviewed materials/ATS fixtures before stored outputs match the active registry |
...
| ATS CV quality | >= 95% pass rate, 0 critical failures | Yellow-Red | A 4-case live NVIDIA materials v3 probe passed automatically, but qualitative review found unsupported serverless component claims in the PSS ATS CV; v4 validator now expands serverless aliases/components | Rerun ATS CV v4 and add DB-backed reviewed ATS CV cases before raising trust |
...
| Application materials | 6.8 | Prompt v7 receives ranking-derived overclaiming constraints as explicit alias families and per-employer technology constraints; recruiter specificity/length gates match the 320-character golden limit; the 4-case v7 live rerun passed automatically and stayed alias-clean. |
| ATS CV | 6.8 | Internal notes, incomplete CVs, omitted base experiences, unsupported avoid-overclaiming aliases, and employer-specific technology drift now have deterministic gates and prompt-level constraints; the 4-case v7 live rerun passed drift-free. |
...
2. Materials and ATS CV v3 had a small fresh automatic generation proof, but qualitative review found a serverless overclaiming false positive; v7 now has a 4-case live rerun, but still needs DB-backed reviewed fixtures.
...
- Materials v7 baseline is run.
- ATS CV v7 baseline is run.
