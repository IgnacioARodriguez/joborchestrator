...
HuntPilot is currently suitable as an operational copilot for job discovery, ranking review, and draft generation. It is not suitable for near-blind trust across the whole product yet. Ranking now uses prompt v9 after the v6/v7/v8 probes exposed evidence omissions and SKIP-vs-AVOID calibration ambiguity. The reviewed ranking baseline measures 22/22 with 0 critical failures, and fresh v9 probe job `#27` completed 30/30 with 0 failed items and 0 critical metrics failures. Materials/ATS CV have stronger controlled-generation guardrails but still need measured DB-backed quality work.
...
- Active materials kit prompt: `materials/nvidia_kit_contract` v14.
...
- Ranking v9 live probe closure: probe ranking job `#27` completed 30/30 saved, 0 failed, prompt trace v9 for all rows, max item attempts 3, and 0 high-attempt items. Metrics over the correct job ranking version measured `unsafe_apply_now_count=0`, `critical_failures=0`, `stale_completion_count=0`, `non_active_prompt_count=0`, `soft_dealbreaker_count=0`, `soft_central_gap_count=0`, and `soft_location_review_count=0`. Decision distribution was 2 APPLY_NOW, 1 APPLY_WITH_TAILORED_CV, 20 SKIP, and 7 AVOID. Both APPLY_NOW rows were synthetic Acme strong-fit cases; the only real positive was job 86 as APPLY_WITH_TAILORED_CV with review flags and explicit gaps.
...
| Materials quality | >= 90% pass rate, 0 critical failures | Yellow-Red | Historical stored evals show 0/3 passing; controlled-generation work adds stricter overclaiming, tone, density, structured-kit, metadata, and export guardrails after the original 4-case v3 probe exposed unsafe serverless claims | Run a fresh controlled-generation materials sample, add DB-backed reviewed materials fixtures, and expand sample before raising trust |
| ATS CV quality | >= 95% pass rate, 0 critical failures | Yellow-Red | A 4-case live NVIDIA materials v3 probe passed automatically, but qualitative review found unsupported serverless component claims in the PSS ATS CV; controlled-generation work now adds alias-family blocking, employer technology attribution, deterministic rendering, and overcompression checks | Run a fresh controlled ATS CV sample and add DB-backed reviewed ATS CV cases before raising trust |
...
| Application materials | 7.1 | Kit v14 uses structured autofill and receives ranking-derived overclaiming constraints, per-employer technology constraints, ATS fit analysis, and stricter exploratory-review tone constraints; validation rejects blank/degenerate cover letters, JSON-encoded autofill strings, internal evaluator language, unsupported hedges, overconfident SKIP/risky-role language, language mismatch, and preserves validation metadata on fail-closed errors. |
...
2. Materials and ATS CV v3 had a small fresh automatic generation proof, but qualitative review found a serverless overclaiming false positive; v10 later had a consolidated 4-case live rerun, while the controlled-generation branch has stronger local guardrails but still needs a fresh live sample and DB-backed reviewed fixtures.
...
- Controlled materials offline benchmark is run; live paired materials baseline is still pending credentials/sample execution.
- Controlled ATS CV offline benchmark is run; live paired ATS CV baseline is still pending credentials/sample execution.
